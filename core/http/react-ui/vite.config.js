...
export default defineConfig(({ mode }) => {
  // COVERAGE=true or --mode coverage produces an instrumented build whose
  // modules report istanbul counters on window.__coverage__, harvested by the
  // Playwright coverage fixture (e2e/coverage-fixtures.js). Off by default so
  // normal/dev/prod builds carry no instrumentation overhead.
  const coverage = process.env.COVERAGE === 'true' || mode === 'coverage'
  // COVERAGE_V8=true or --mode coverage-v8 produces a NON-instrumented build
  // with source maps, so the Playwright coverage fixture can collect Chromium
  // V8 coverage and map it back to source via v8-to-istanbul. Mutually
  // exclusive with COVERAGE.
  const coverageV8 = process.env.COVERAGE_V8 === 'true' || mode === 'coverage-v8'
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        // The coverage build bundles all dynamic imports into a single chunk.
